...
public class PartiesApplication extends Application {
    ...
    public PartiesApplication() {
        this.values = fileLoader("partiesdata.tsv");
    }
    
    private HashMap<String, HashMap<Integer, Double>> fileLoader(String filename){
        ...
        try(Scanner data = new Scanner(Paths.get(filename))) {
            ...
            for (int i = 1; i < stringArr.length; i++) {
                yearList.add(Integer.valueOf(stringArr[i]));
            }
            ...
            while (data.hasNext()) {
                ...
                for (int i = 1; i < stringArr.length; i++) {
                    if (!stringArr[i].equals("-")) {
                        ...
                        double number = Double.valueOf(dat);
                        map.put(year, number);
                    }
                }
